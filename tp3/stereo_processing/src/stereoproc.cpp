...
static void extract_features(const cv::Mat& imgLeft, const cv::Mat& imgRight, std::vector<cv::KeyPoint>& keypoints_l, std::vector<cv::KeyPoint>& keypoints_r, cv::Mat& descriptors_l, cv::Mat& descriptors_r) {
    int nfeatures = 500;
...
static void display_matches(cv::Mat rectifiedLeft, std::vector<cv::KeyPoint> keypoints1, cv::Mat rectifiedRight, std::vector<cv::KeyPoint> keypoints2, std::vector<cv::DMatch> inlierMatches, bool block) {
    cv::Mat imgMatches;
...
void process_images(cv::Mat& imgLeft, cv::Mat& imgRight, cv::Mat& R_estimated, cv::Mat& T_estimated, std::vector<cv::Point3d>& points3D, bool block) {
    ...
    cv::Mat descriptors1;
    cv::Mat descriptors2;
    extract_features(rectifiedLeft, rectifiedRight, keypoints1, keypoints2, descriptors1, descriptors2);
    ...
    for (const auto& match : inlierMatches) {
        ...
        inlierPointsLeft.push_back(keypoints1[match.queryIdx].pt);
        inlierPointsRight.push_back(keypoints2[match.trainIdx].pt);
    }
