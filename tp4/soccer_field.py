...
class Field:
    ...
    def G(self, x, u):
        ...
        g1(xt, ut) = x + trans * np.cos(theta + rot1)
        g2(xt, ut) = y + trans * np.sin(theta + rot1)
        g3(xt, ut) = theta + rot1 + rot2

        G = [
            dg1/dx, dg1/dy, dg1/dtheta
            dg2/dx, dg2/dy, dg2/dtheta
            dg3/dx, dg3/dy, dg3/dtheta
        ]
        """
        G = np.array([
    ...
    def V(self, x, u):
        ...
        V = [
            dg1/drot1, dg1/dtrans, dg1/drot2
            dg2/drot1, dg2/dtrans, dg2/drot2
            dg3/drot1, dg3/dtrans, dg3/drot2
        ]
        """
        V = np.array([
    ...
    def H(self, x, marker_id):
        ...
        h(xt, mt) = np.arctan2(y_m - y, x_m - x) - theta

        H = [dh/dx, dh/dy, dh/dtheta]
        """
        m_x = self.MARKER_X_POS[marker_id]
        m_y = self.MARKER_Y_POS[marker_id]

        q = (m_x - prev_x)**2 + (m_y - prev_y)**2

        H = np.array([
            (m_y - prev_y) / q, -(m_x - prev_x) / q, -1
        ]).reshape((1, 3))
